...
void ProcessPacket(int ci, unsigned char packet[])
{
	...
	case SC_MOVE_PLAYER: {
		...
		if (move_packet->id < MAX_CLIENTS) {
			...
			// 자신이 보내 move 패킷을 받은경우
			if (ci == my_id) {
				// 보냈을때 시간과 다시 받은 시간의 차이를 이용해 딜레이를 계산함
	...
	case SC_LOGIN_INFO:
	{
		...
		int my_id = ci;
		// login->id  클라이언트의 어느세션인지 매핑위해 이걸 만들어준 매핑 테이블
...
void Test_Thread()
{
	while (true) {
		...
		Adjust_Number_Of_Client();	// 천천히 동접을 올리는 용도

		// 1초 한번씩만 이동패킷을 보내기 위한 코드
