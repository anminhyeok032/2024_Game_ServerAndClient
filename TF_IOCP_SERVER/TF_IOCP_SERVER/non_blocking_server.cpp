...
void recv_callback(LPWSAOVERLAPPED _recv_over, int key);
...
public:
	...
	void do_recv(int key)
	{
		...
		int res = WSARecv(_socket, &_recv_over._wsabuf, 1, 0, &recv_flag,
			&_recv_over._over, 0);
		cout << "recv" << endl;
		if (0 == res)
		{
			
			recv_callback(&_recv_over._over, key);
		}
...
void recv_callback(LPWSAOVERLAPPED _recv_over, int key)
{
	...
	int remain_data = ex_over->_wsabuf.len + clients[key]._prev_remain;
...
int main()
{
	...
	while (true) {
		...
		if (client = WSAAccept(server, reinterpret_cast<sockaddr*>(&cl_addr), &addr_size, NULL, NULL) != INVALID_SOCKET)
		{
			unsigned long noblock = 1;
